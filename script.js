...
function showPage(targetId) {
    ...
    if(targetId === 'processContainer') {
        ...
        if(Object.keys(savedImages).length > 0 && processImageGallery) processImageGallery.style.display = 'grid';
       
    } else {
        // ✅ Hide buttons in all other pages
...
function updateProcessButtons() {
    ...
    if (currentPage && currentPage.id === 'processContainer') {
        addRoundBtn.style.display = 'inline-block';
        deployBtn.style.display = 'inline-block';
    } else {
        addRoundBtn.style.display = 'none';
...
// Update buttons whenever navigation happens
navLinks.forEach(link => {
    link.addEventListener('click', () => {
        setTimeout(updateProcessButtons, 10);
    });
});

// Update when clicking Process button
